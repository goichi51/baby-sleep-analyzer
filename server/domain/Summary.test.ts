...
describe('summarize', () => {
  test('startAt 以前に入眠、endAt 以降に覚醒（途中覚醒あり)', () => {
    ...
    const input = new ChildcareLogCollection(events, [])
  ...
  test('startAt 以前に入眠、endAt 以降に覚醒（途中覚醒なし)', () => {
    const input = new ChildcareLogCollection([], [])
    const summary = new Summary(input, startAt, endAt).computeNightSleep()
    expect(summary.sleepSession).toEqual([
      {
        start: new Date('2026-07-25T22:00:00Z'),
...
describe('score', () => {
  ...
  test('総睡眠時間が同じでも覚醒回数が少ないほうがスコアが高い', () => {
    ...
    const badSleepLog = new ChildcareLogCollection(badSleepEvents, [])
    ...
    const goodSleepLog = new ChildcareLogCollection(goodSleepEvents, [])
  ...
  test('総睡眠時間が同じでも長い睡眠睡眠ブロックがあるほうがスコアが高い', () => {
    ...
    const sleepLog = new ChildcareLogCollection(sleepEvents, [])
